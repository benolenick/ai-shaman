...
class AiShamanClient:
    ...
    def queue_status(self) -> dict[str, Any]:
        res = _http_json("GET", f"{self.management_url}/queue/status")
        if res.status != 200:
            raise CliError(f"Queue status failed: {res.status} {res.data}")
        return res.data

    def queue_stats(self) -> dict[str, Any]:
        res = _http_json("GET", f"{self.management_url}/queue/stats")
        if res.status != 200:
            raise CliError(f"Queue stats failed: {res.status} {res.data}")
        return res.data

    def queue_pause(self) -> dict[str, Any]:
        res = _http_json("POST", f"{self.management_url}/queue/pause")
        if res.status != 200:
            raise CliError(f"Queue pause failed: {res.status} {res.data}")
        return res.data

    def queue_resume(self) -> dict[str, Any]:
        res = _http_json("POST", f"{self.management_url}/queue/resume")
        if res.status != 200:
            raise CliError(f"Queue resume failed: {res.status} {res.data}")
        return res.data

    def queue_clear(self, priority: str) -> dict[str, Any]:
        res = _http_json("POST", f"{self.management_url}/queue/clear", {"priority": priority})
        if res.status != 200:
            raise CliError(f"Queue clear failed: {res.status} {res.data}")
        return res.data

    def queue_submit(self, model: str, prompt: str, priority: str = "normal",
                     consumer: str = "cli", endpoint: str = "/api/generate",
                     timeout: float = 300.0, extra: dict | None = None) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "model": model,
            "prompt": prompt,
            "priority": priority,
        ...
        res = _http_json("POST", f"{self.management_url}/queue/submit", payload, timeout=30.0)
        if res.status not in {200, 202}:
            raise CliError(f"Queue submit failed: {res.status} {res.data}")
        return res.data

    def queue_result(self, job_id: str) -> dict[str, Any]:
        res = _http_json("GET", f"{self.management_url}/queue/result/{job_id}")
        if res.status not in {200, 202}:
            raise CliError(f"Queue result failed: {res.status} {res.data}")
        return res.data

    def queue_wait(self, job_id: str, poll_interval: float = 2.0,
                   max_wait: float = 600.0) -> dict[str, Any]:
        """Poll for result until done or timeout."""
        deadline = time.time() + max_wait
        while time.time() < deadline:
            result = self.queue_result(job_id)
            state = result.get("state", "")
            if state in {"done", "error", "cancelled"}:
                return result
            time.sleep(poll_interval)
        raise CliError(f"Timed out waiting for job {job_id} after {max_wait}s")
...
def _print_queue_status(status: dict[str, Any]) -> None:
    print(f"Queue paused: {status.get('paused', False)}")
    print(f"Queue depth: {status.get('queue_depth', 0)}")
    by_p = status.get("pending_by_priority", {})
    print(f"  critical: {by_p.get('critical', 0)}")
    print(f"  normal:   {by_p.get('normal', 0)}")
    print(f"  bulk:     {by_p.get('bulk', 0)}")
    active = status.get("active_job")
    if active:
        print(f"Active job: {active.get('job_id')} [{active.get('priority')}] consumer={active.get('consumer')} model={active.get('model')}")
    else:
        print("Active job: (none)")
    pending = status.get("pending_jobs", [])
    if pending:
        print(f"Pending jobs ({len(pending)}):")
        for j in pending[:10]:
            print(f"  {j.get('job_id', '')[:12]}... [{j.get('priority')}] {j.get('consumer')} {j.get('model')}")
        if len(pending) > 10:
            print(f"  ... and {len(pending) - 10} more")


def _print_queue_stats(stats: dict[str, Any]) -> None:
    print(f"Total jobs submitted: {stats.get('total_jobs', 0)}")
    print("By priority:")
    for p, s in stats.get("by_priority", {}).items():
        print(f"  {p:8s}: submitted={s.get('submitted',0)} done={s.get('done',0)} errors={s.get('errors',0)} avg_latency={s.get('avg_latency_s',0)}s")
    print("By consumer:")
    for c, s in stats.get("by_consumer", {}).items():
        print(f"  {c:12s}: submitted={s.get('submitted',0)} done={s.get('done',0)} errors={s.get('errors',0)} avg_latency={s.get('avg_latency_s',0)}s")


def _interactive_queue_menu(client: AiShamanClient) -> None:
    print("\n-- Job Queue --")
    print("1. Queue status")
    print("2. Queue stats")
    print("3. Pause queue")
    print("4. Resume queue")
    print("5. Clear bulk jobs")
    print("6. Submit test job")
    print("7. Back")
    choice = input("Choose [1-7]: ").strip()
    try:
        if choice == "1":
            _print_queue_status(client.queue_status())
        elif choice == "2":
            _print_queue_stats(client.queue_stats())
        elif choice == "3":
            _print_result(client.queue_pause(), as_json=False)
        elif choice == "4":
            _print_result(client.queue_resume(), as_json=False)
        elif choice == "5":
            confirm = input("Clear all pending bulk jobs? [y/N]: ").strip().lower()
            if confirm in {"y", "yes"}:
                _print_result(client.queue_clear("bulk"), as_json=False)
        elif choice == "6":
            model = input("Model (e.g. qwen3.5:latest): ").strip()
            prompt = input("Prompt: ").strip()
            priority = input("Priority [critical/normal/bulk] (default normal): ").strip().lower() or "normal"
            consumer = input("Consumer name (default cli): ").strip() or "cli"
            sub = client.queue_submit(model=model, prompt=prompt, priority=priority, consumer=consumer)
            print(f"Submitted: job_id={sub.get('job_id')}")
            if input("Wait for result? [y/N]: ").strip().lower() in {"y", "yes"}:
                result = client.queue_wait(sub["job_id"])
...
def _interactive_menu(client: AiShamanClient) -> int:
    while True:
        ...
        print("7. Job queue management")
        print("8. Exit")
        try:
            choice = input("Choose [1-8] (or q to quit): ").strip().lower()
        ...
        try:
            ...
            if choice == "1":
                ...
                _interactive_queue_menu(client)
            else:
                print("Invalid choice.")
        except Exception as e:
            print(f"Error: {e}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # Job queue subcommand
    queue = sub.add_parser("queue", help="Manage the priority job queue", parents=[common])
    queue_sub = queue.add_subparsers(dest="queue_cmd", required=True)

    queue_sub.add_parser("status", help="Show queue state (pending jobs, active job, depth by priority)", parents=[common])
    queue_sub.add_parser("stats", help="Show throughput, avg latency, jobs by consumer", parents=[common])
    queue_sub.add_parser("pause", help="Pause queue processing (no new jobs dispatched)", parents=[common])
    queue_sub.add_parser("resume", help="Resume queue processing", parents=[common])

    q_clear = queue_sub.add_parser("clear", help="Cancel all pending jobs of a given priority", parents=[common])
    q_clear.add_argument("--priority", required=True, choices=["critical", "normal", "bulk"],
                         help="Priority tier to clear")

    q_submit = queue_sub.add_parser("submit", help="Submit a test/manual job to the queue", parents=[common])
    q_submit.add_argument("--model", required=True, help="Model name (e.g. qwen3.5:latest)")
    q_submit.add_argument("--prompt", required=True, help="Prompt text")
    q_submit.add_argument("--priority", default="normal", choices=["critical", "normal", "bulk"])
    q_submit.add_argument("--consumer", default="cli", help="Consumer name tag")
    q_submit.add_argument("--endpoint", default="/api/generate",
                          choices=["/api/generate", "/api/chat", "/v1/chat/completions"])
    q_submit.add_argument("--timeout", type=float, default=300.0, help="Job timeout in seconds")
    q_submit.add_argument("--wait", action="store_true", help="Wait for result and print it")

    q_result = queue_sub.add_parser("result", help="Poll result for a submitted job", parents=[common])
    q_result.add_argument("job_id", help="Job ID returned from submit")
    q_result.add_argument("--wait", action="store_true", help="Poll until done")
        ...
        if args.cmd == "status":
            ...
            if args.queue_cmd == "status":
                ...
                if as_json:
                    ...
                    _print_queue_status(qs)
                    return 0
            elif args.queue_cmd == "stats":
                qs = client.queue_stats()
                if as_json:
                    res = qs
                else:
                    _print_queue_stats(qs)
                    return 0
            elif args.queue_cmd == "pause":
                res = client.queue_pause()
            elif args.queue_cmd == "resume":
                res = client.queue_resume()
            elif args.queue_cmd == "clear":
                res = client.queue_clear(args.priority)
            elif args.queue_cmd == "submit":
                sub_res = client.queue_submit(
                    model=args.model,
                    prompt=args.prompt,
                ...
                if args.wait:
                    ...
                    if not job_id:
                        raise CliError("Submit returned no job_id")
                    res = client.queue_wait(job_id)
                else:
                    res = sub_res
            elif args.queue_cmd == "result":
                if args.wait:
                    res = client.queue_wait(args.job_id)
                else:
                    res = client.queue_result(args.job_id)
            else:
                raise CliError(f"Unsupported queue command: {args.queue_cmd}")
        else:
